...
(function (jQuery) {
    ...
    var nowMorth=(new Date()).getMonth();
	...
	var Chart = function(div, opts) {
        ...
        function addVtHeader(div, data, cellHeight) {
            ...
            for (var i = 0; i < data.length; i++) {
                var itemDiv = jQuery("<div>", { "class": "gantt_grid_data" });
               var rowDiv=jQuery("<div>", { "class": "gantt_row",
                    "css": { "height": "32px","line-height":"32px" } });
                var cellDiv=jQuery("<div>", { "class": "gantt_cell"  ,
                    "css": { "width": "300px"}});
                var cellInfoDiv=jQuery("<div>", { "class": "gantt_tree_icon gantt_blank" })
                    .after(jQuery("<div>", { "class": "gantt_tree_content" ,
                        "css": { "width": "80px"}})
                        .append(data[i].name));

                var cellInfoDiv1=jQuery("<div>", { "class": "gantt_tree_icon gantt_blank" })
                    .after(jQuery("<div>", { "class": "gantt_tree_content",
                      "css": { "width": "70px","text-align":"center"} }).append(data[i].start));
                var cellInfoDiv2=jQuery("<div>", { "class": "gantt_tree_icon gantt_blank" })
                    .after(jQuery("<div>", { "class": "gantt_tree_content",
                        "css": { "width": "70px","text-align":"center"} }).append(data[i].end));

                cellDiv.append(cellInfoDiv).append(cellInfoDiv1).append(cellInfoDiv2);
                rowDiv.append(cellDiv);
                itemDiv.append(rowDiv);
                var seriesDiv = jQuery("<div>", { "class": "gantt_grid_data" });
                for (var j = 0; j < data[i].series.length; j++) {
                    ...
                    var cellInfoDiv=(jQuery("<div>", { "class": "gantt_tree_icon gantt_file" }))
                        .after(jQuery("<div>", { "class": "gantt_tree_content" ,
                            "css": { "width": "70px"}})
                    ...
                    var cellInfoDiv1=jQuery("<div>", { "class": "gantt_tree_icon gantt_blank" })
                        .after(jQuery("<div>", { "class": "gantt_tree_content",
                        "css": { "width": "70px","text-align":"center"} }).append($.toShortDate($.toDateTime(data[i].series[j].end))));

                    var cellInfoDiv2=jQuery("<div>", { "class": "gantt_tree_icon gantt_blank" })
                        .after(jQuery("<div>", { "class": "gantt_tree_content" }).append($.toShortDate($.toDateTime(data[i].series[j].end))));
			...
			for (var y in dates) {
				for (var m in dates[y]) {
					for (var d in dates[y][m]) {
                        ...
                        if(y==nowYear && m==nowMorth && d==nowDay-1)
                        {
                            cellDiv.append(jQuery("<hr size=100 width='1' style='height:28px;margin-top:0px;' color='red'>"));
                        }
						rowDiv.append(cellDiv);
					}
				}
        ...
        //根据订单的开始时间结束时间画已完成部分
        function addBlocksWc(div, data, cellWidth, start) {
            var rows = jQuery("div.ganttview-blocks div.ganttview-block-container", div);
            var rowIdx = 0;
            for (var i = 0; i < data.length; i++) {
                for (var j = 0; j < data[i].series.length; j++) {
	...
	var Behavior = function (div, opts) {
        ...
        function bindBlockdblClick(div, callback) {
            jQuery("div.ganttview-block", div).live("dblclick", function () {
                if (callback) { callback(jQuery(this).data("block-data")); }
            });
        }
